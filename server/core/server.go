...
func NewServer(cfg *config.Config) (srv.Startable, error) {
	...
	grpcServer, err := NewGrpcServer(gs, cfg.SrvAddr)
	if err != nil {
		return nil, err
	}
	streamsServer, err := NewStreamsServer(streamsManager)
	if err != nil {
		return nil, err
	}
	transactionsServer, err := NewTransactionsServer(transactionsManager)
	if err != nil {
		return nil, err
	}
